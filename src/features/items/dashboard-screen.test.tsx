...
describe('DashboardScreen', () => {
  ...
  it('renders cover display as cover-only tiles with top-left action controls and accessible fallbacks', async () => {
    ...
    expect(coversGrid).toHaveClass('columns-2', 'gap-4', 'lg:columns-3', 'xl:columns-4')
    expect(coversGrid).not.toHaveClass('grid', 'auto-rows-[8px]', 'grid-cols-2', 'lg:grid-cols-3', 'xl:grid-cols-3', 'xl:grid-cols-4')
    expect(screen.queryByTestId('dashboard-cards-grid')).not.toBeInTheDocument()
    expect(screen.queryByTestId('dashboard-list')).not.toBeInTheDocument()
    const coverArticles = await screen.findAllByRole('article')

    expect(coverArticles).toHaveLength(3)
    ...
    expect(coverArticles.every((article) => article.classList.contains('break-inside-avoid'))).toBe(true)
    expect(coverArticles.every((article) => article.classList.contains('mb-4'))).toBe(true)
    expect(screen.getByTestId('dashboard-cover-fallback')).toBeInTheDocument()
    expect(screen.getByRole('link', { name: 'Edit: Severance' }).closest('article')).not.toHaveClass('row-span-[15]')
    expect(screen.getByRole('link', { name: 'Edit: Arrival' }).closest('article')).not.toHaveClass('row-span-[15]')
    expect(screen.getByRole('link', { name: 'Edit: Atomic Habits' }).closest('article')).not.toHaveClass('row-span-[16]')
