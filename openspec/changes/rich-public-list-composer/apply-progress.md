...
| `npx pnpm test src/features/items/pocketbase-public-list-repository.test.ts` | Passed after PocketBase date payload fix: 21 tests |
| `npx pnpm typecheck` | Passed after PocketBase date payload fix |
...
- `src/features/items/pocketbase-public-list-repository.ts` — tightened managed-list queries to include authenticated owner ownership while preserving public DTO projections; create payloads omit absent optional text fields instead of sending `null`; PocketBase date payloads include a time component.
- `src/features/items/pocketbase-public-list-repository.test.ts` — added/updated owner filtering coverage for managed public-list queries plus create payload optional text/date regression coverage.
...
- Live PocketBase create rejected payloads with generic 400 responses; create payloads now omit absent optional text fields and serialize PocketBase date fields with a time component.
