## Exploration: Implement shadcn/ui Kyoumi web app
...
- Primary categories: `series`, `movies`, `games`, `books`, `music`, `podcasts`.
- MVP screens/behaviors: dashboard, responsive mobile dashboard behavior, adaptive add flow, and ES/EN language toggle.
...
- `untitled.op` — OpenPencil design source already adapted toward shadcn/ui and the adaptive add-flow direction.
...
1. **Bootstrap a modern React app with shadcn/ui first** — Create the web foundation, shadcn/ui configuration, styling tokens, and static MVP UI using local mock data.
   ...
   - Cons: Requires choosing the framework, package manager, and test/quality tools before feature work; browser-local persistence still needs a clear boundary.
   - Effort: Medium
...
### Recommendation
Use approach 1: bootstrap a modern React-based web app with shadcn/ui and deliver the first MVP UI slice with mock data. The recommended first slice is the dashboard shell plus category cards, responsive mobile navigation, ES/EN toggle, and an adaptive add flow surfaced through a `Sheet` on mobile and `Dialog` or routed form on desktop.
...
- Category IDs: `series`, `movies`, `games`, `books`, `music`, `podcasts`.
- Shared fields: `id`, `category`, `title`, `notes`, `status`, `createdAt`, optional `tags`.
- Status values: `pending`, `in_progress`, `completed`.
- Shared add fields should stay category-agnostic at the MVP layer, with room for category-specific metadata later.
- Category actions should map to content semantics, e.g. watched for series/movies, read for books, and listened for music/podcasts.
...
### Ready for Proposal
Yes. The next phase should create a proposal for `implement-shadcn-meinteresa-ui` that scopes the first change to app scaffold + shadcn/ui + static/mock-data MVP UI for dashboard, mobile dashboard behavior, adaptive add flow, category taxonomy, and ES/EN toggle. The proposal should explicitly defer auth, database persistence, archive analytics, and Gemini recommendations unless the user chooses to include them.
